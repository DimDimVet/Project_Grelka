...
uint8_t Survey_Key(Key_Structure *Key_Str)
{
	...
	for(int i = 0; i < (Key_Str->cols_count);i++)
	{
		ODR_1(&Key_Str->pins_col[i]);
			...
			for(int i = 0; i < (Key_Str->rows_count);i++)
			{
				temp =(Key_Str->pins_rows[i].GPIOx->IDR)& (1 << Key_Str->pins_rows[i].Pin);
				//temp = Read_BIT((Key_Str->pins_rows[i].GPIOx->IDR),(1 << Key_Str->pins_rows[i].Pin));
				if(temp != 0)
				{
						TempChar("SDF1");
				}
